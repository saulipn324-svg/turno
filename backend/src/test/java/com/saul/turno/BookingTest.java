...
class BookingTest {
 ...
 Draft draft(String room,int start,int duration){return new Draft(room,date,start,duration,"Planeación","Equipo");}
 ...
 @Test void rejectsOverlapAndRollsBack(){service.create(draft("norte",600,60));assertThrows(BookingConflict.class,()->service.create(draft("norte",570,60)));assertEquals(1,service.list(date).size());assertEquals(2,db.queryForObject("SELECT COUNT(*) FROM slots",Integer.class));}
 ...
 @Test void rejectsPastAndBadDate(){assertThrows(IllegalArgumentException.class,()->service.create(new Draft("norte","2020-01-01",600,60,"Test","Equipo")));assertThrows(IllegalArgumentException.class,()->service.create(new Draft("norte","2027-02-30",600,60,"Test","Equipo")));}
 @Test void simultaneousReservationsHaveOneWinner()throws Exception{try(var pool=Executors.newFixedThreadPool(2)){var gate=new CountDownLatch(1);Callable<Boolean> request=()->{gate.await();try{service.create(draft("norte",600,90));return true;}catch(BookingConflict e){return false;}};var a=pool.submit(request);var b=pool.submit(request);gate.countDown();assertNotEquals(a.get(15,TimeUnit.SECONDS),b.get(15,TimeUnit.SECONDS));assertEquals(1,service.list(date).size());assertEquals(3,db.queryForObject("SELECT COUNT(*) FROM slots",Integer.class));}}
}
